...
def main():
    parser = argparse.ArgumentParser(
        description='Load a trained Spikformer checkpoint, prune attention heads, and save pruned model.'
    )
    parser.add_argument('--config', type=str, required=True,
                        help='Path to YAML config file used for training (e.g. cifar10.yml)')
    parser.add_argument('--checkpoint', type=str, required=True,
                        help='Path to trained checkpoint (.pth.tar)')
    parser.add_argument('--output', type=str, required=True,
                        help='Path to save pruned checkpoint')
    parser.add_argument('--rank-ratio', type=float, default=0.5,
                        help='Fraction of attention heads to keep per layer (e.g. 0.5)')
    ...
    model = create_model('spikformer', **cfg)
    
    # Load weights
    load_checkpoint(model, args.checkpoint, use_ema=False)
    print(f"✅ Loaded checkpoint '{args.checkpoint}'")

    # Prune heads
    keep_heads = prune_spikformer_heads(model, rank_ratio=args.rank_ratio)
    print(f"✅ Pruned model: kept {args.rank_ratio*100:.1f}% heads per block")

    # Save pruned model state
    torch.save({
        'model_state': model.state_dict(),
        'keep_heads_per_block': keep_heads
    }, args.output)
